...
console.log(
  "This script populates some sample nudges and participants to your database. Check out the sample data at ./sampleData.js"
);
...
async function checkParticipantExists() {
  ...
  const participantIds = sampleData.participantData.map(
    (item) => item.participantId
  );
  for (const participantId of participantIds) {
    try {
      const response = await fetch(
        `http://127.0.0.1:5000/api/data_objects/${participantId}`
      );
      ...
      console.error(
        `Error querying participant ID ${participantId}: ${error.message}`
      );
    }
  }
}
